...
| `--reasoning` | `off` | Disabled — the 4B model over-thinks at 2.8 tok/s, burning tokens on `<think>` blocks before producing content. Set to `on` if you want extended reasoning (needs high `max_tokens` and patience). |
